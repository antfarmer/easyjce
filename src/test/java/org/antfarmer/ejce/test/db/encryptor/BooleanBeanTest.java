...
import org.antfarmer.ejce.test.db.encryptor.bean.BooleanBean;
 ...
 * @author Ameer Antar
 */
public class BooleanBeanTest extends AbstractDbTest {

	private static final Class<?> BEAN_CLASS = BooleanBean.class;
	...
	public void test() {
		...
		execute(new HibernateCallback() {
			...
			public void doInHibernate(final Session session) {
				BooleanBean sb = (BooleanBean) session.get(BEAN_CLASS, id);
				assertNull(sb);
				sb = new BooleanBean(value);
				saveOrUpdate(sb);
			}
		});
		...
		execute(new HibernateCallback() {
			@Override
			public void doInHibernate(final Session session) {
				final BooleanBean sb = (BooleanBean) session.get(BEAN_CLASS, id);
				assertNotNull(sb);
