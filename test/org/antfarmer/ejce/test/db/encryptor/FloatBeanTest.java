...
import org.antfarmer.ejce.test.db.encryptor.bean.FloatBean;
 ...
 * @author Ameer Antar
 */
public class FloatBeanTest extends AbstractDbTest {

	private static final Class<?> BEAN_CLASS = FloatBean.class;

	@Test
	public void test() {

		final long id = 1;
		final Float value = 3.3f;

		execute(new HibernateCallback() {
			@Override
			public void doInHibernate(final Session session) {
				FloatBean sb = (FloatBean) session.get(BEAN_CLASS, id);
				assertNull(sb);
				sb = new FloatBean(value);
				saveOrUpdate(sb);
			}
		});
		...
		execute(new HibernateCallback() {
			@Override
			public void doInHibernate(final Session session) {
				final FloatBean sb = (FloatBean) session.get(BEAN_CLASS, id);
				assertNotNull(sb);
