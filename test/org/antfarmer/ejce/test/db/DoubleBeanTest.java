...
import org.antfarmer.ejce.test.db.bean.DoubleBean;
 ...
 * @author Ameer Antar
 */
public class DoubleBeanTest extends AbstractDbTest {

	private static final Class<?> BEAN_CLASS = DoubleBean.class;

	@Test
	public void test() {

		final long id = 1;
		final Double value = 3.3;

		execute(new HibernateCallback() {
			@Override
			public void doInHibernate(final Session session) {
				DoubleBean sb = (DoubleBean) session.get(BEAN_CLASS, id);
				assertNull(sb);
				sb = new DoubleBean(value);
				saveOrUpdate(sb);
			}
		});
		...
		execute(new HibernateCallback() {
			@Override
			public void doInHibernate(final Session session) {
				final DoubleBean sb = (DoubleBean) session.get(BEAN_CLASS, id);
				assertNotNull(sb);
